...
with tab6:
    ...
    if st.button("📥 加载历史日志", type="secondary"):
        try:
            token = get_token()
            headers_r = {"Authorization": f"Bearer {token}"}
            items = []
            page_token_r = None
            while True:
                params = {"page_size": 20}
                if page_token_r: params["page_token"] = page_token_r
                r = requests.get(
                    f"https://open.feishu.cn/open-apis/bitable/v1/apps/{REVIEW_BASE}/tables/{REVIEW_TABLE}/records",
                    params=params, headers=headers_r, timeout=15, verify=False)
                d = r.json()
                if d.get("code") != 0: break
                items.extend(d.get("data", {}).get("items", []))
                if d.get("data", {}).get("has_more"):
                    page_token_r = d["data"]["page_token"]
                else: break

            if "review_logs" not in st.session_state:
                st.session_state.review_logs = []
            st.session_state.review_logs = items
            st.success(f"已加载 {len(items)} 条日志")
        except Exception as e:
            st.error(f"加载失败: {e}")

    logs = st.session_state.get("review_logs", [])
    if logs:
        logs.sort(key=lambda x: x.get("fields", {}).get("时间", 0), reverse=True)
        for log in logs[:5]:
            f = log["fields"]
            ts = f.get("时间", 0)
            try:
    ...
    with st.form("review_form"):
        ...
        rev_to_improve = st.text_area("🎯 下场需要改进/新增的点", height=80,
                                      placeholder="下次重点练什么...")
        rev_learned = st.text_area("💡 今日学习点/发现", height=80,
                                   placeholder="新发现的话术技巧/用户痛点...")

        if st.form_submit_button("💾 提交到飞书"):
            if not rev_good and not rev_to_improve:
                st.warning("至少填写一项")
            else:
                try:
                    token = get_token()
                    ts_ms = int(datetime.combine(rev_date, datetime.min.time()).timestamp() * 1000)
                    ...
                    if resp.json().get("code") == 0:
                        st.success("✅ 已提交到飞书！")
                        st.session_state.pop("review_logs", None)
                    else:
                        st.error(f"提交失败: {resp.json().get('msg')}")
                except Exception as e:
                    st.error(f"提交失败: {e}")
